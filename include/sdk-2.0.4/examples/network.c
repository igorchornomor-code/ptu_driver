...
void print_str(struct cerial *cer, const char *desc, cpi_opcode op){
    uint16_t    status;
    char        str[64];

    if(cpi_ptcmd(cer, &status, op, (int)sizeof(str), NULL, &str)){
        die("%s get failed\n", desc);
    }
...
int main(int argc, char *argv[]){
    ...
    print_str(cer, "IP", OP_NET_IP_GET);
    print_str(cer, "Gateway", OP_NET_GATEWAY_GET);
